...
SCRIPT="${ROOT}/scripts/synthetic_maze2d_sac_her_probe.py"
LD_MJ="/root/.mujoco/mujoco210/bin"

STAMP="$(date +%Y%m%d-%H%M%S)"
SESSION="maze2d_sac_her_${STAMP}"

SMOKE="${SMOKE:-0}"
SEED="${SEED:-0}"
MONITOR_TAG="${MONITOR_TAG:-sac_her}"
REWARD_MODE="${REWARD_MODE:-sparse}" # sparse|shaped
LOGDIR="${LOGDIR:-}" # optional explicit --logdir

# Fair-comparison defaults (aligned to current diffuser auto-decider baseline family).
ENV_NAME="${ENV_NAME:-maze2d-umaze-v1}"
...
SAC_HIDDEN_DIMS="${SAC_HIDDEN_DIMS:-256,256}"
SAC_AUTO_ALPHA="${SAC_AUTO_ALPHA:-1}"
SAC_ALPHA="${SAC_ALPHA:-0.2}"
HER_K_PER_TRANSITION="${HER_K_PER_TRANSITION:-4}"
HER_FUTURE_SAMPLE_ATTEMPTS="${HER_FUTURE_SAMPLE_ATTEMPTS:-16}"
...
CMD=(
  ...
  "--sac_alpha '${SAC_ALPHA}'"
  "--her_k_per_transition '${HER_K_PER_TRANSITION}'"
  "--her_future_sample_attempts '${HER_FUTURE_SAMPLE_ATTEMPTS}'"
)
...
if [[ "${SAC_AUTO_ALPHA}" == "1" ]]; then
  CMD+=("--sac_auto_alpha")
else
  CMD+=("--no_sac_auto_alpha")
fi
